...
def plot_residential_demand_map(grid_gdf, col_to_plot, app_config, admin_gdf_param, region_gdf_param, fig_size=(25, 15)):
    ...
    grid_display.sort_values(col_to_plot, ascending=True).plot(
        ...
        legend_kwds={"label": "Residential Consumption (MWh per cell)"})
    ...
    ax.set_title(f'Residential Electricity Consumption in {app_config.AREA_OF_INTEREST} ({col_to_plot}, MWh)')
    ...
    plt.savefig(app_config.RESIDENTIAL_OUTPUT_DIR / f'map_residential_demand_log_{app_config.COUNTRY}.png', bbox_inches='tight')
    # plt.show()


def plot_service_demand_map(grid_gdf, col_to_plot, app_config, admin_gdf_param, region_gdf_param, fig_size=(25, 15)):
    """
    Plots the Service Electricity Demand on a map with a logarithmic scale.
    """
    ...
    grid_display = grid_gdf.copy()
    grid_display[col_to_plot] = grid_display[col_to_plot] / 1000 # kWh to MWh for display

    fig, ax = plt.subplots(figsize=fig_size)
    ax.set_xlabel('Longitude (°)')
    ax.set_ylabel('Latitude (°)')

    # --- Copied from residential plot for robust v_min/v_max ---
    v_max = grid_display[col_to_plot].max() if not grid_display[col_to_plot].empty else 1.0
    # Create a series of only the positive values from the column
    positive_values = grid_display[col_to_plot][grid_display[col_to_plot] > 0]
    # If there are any positive values, find the minimum. Otherwise, use a default small number.
    v_min = positive_values.min() if not positive_values.empty else 1e-6

    grid_display.sort_values(col_to_plot, ascending=True).plot(
        ax=ax, column=col_to_plot, cmap="Reds", legend=True, alpha=0.9,
        norm=colors.LogNorm(vmin=v_min, vmax=v_max),  # Using LogNorm
        legend_kwds={"label": "Service Consumption (MWh per cell)"})

    if admin_gdf_param is not None:
        admin_gdf_param.to_crs(grid_gdf.crs).plot(ax=ax, edgecolor='brown', facecolor='None', alpha=0.6)
    if region_gdf_param is not None:
        region_gdf_param.to_crs(grid_gdf.crs).plot(ax=ax, edgecolor='brown', facecolor='None', alpha=0.2)

    ax.set_aspect('equal', 'box')
    ax.set_title(f'Service Electricity Consumption in {app_config.AREA_OF_INTEREST} ({col_to_plot}, MWh)')

    points = gpd.GeoSeries([Point(-73.5, 40.5), Point(-74.5, 40.5)], crs=app_config.CRS_WGS84)
    points = points.to_crs(32619)
    distance_meters = points[0].distance(points[1])
    scalebar = ScaleBar(distance_meters, dimension="si-length", location='lower left', length_fraction=0.1, width_fraction=0.001, units='m', color='black')
    ax.add_artist(scalebar)

    plt.savefig(app_config.RESIDENTIAL_OUTPUT_DIR / f'map_service_demand_log_{app_config.COUNTRY}.png', bbox_inches='tight')
    # plt.show()