...
wealth_index = 1e-5*data["Wealth index factor score for urban/rural (5 decimals)"].to_numpy(float)
weight = 1e-5*data['Household sample weight (6 decimals)'].to_numpy(float)
energy_use = data["Energy Use"].to_numpy(float)
province = data['Province'].to_numpy(int)
...
infile = '../Data/zambia_subregions.csv' # Read file containing the estimated mean wealth index ("rwi") of each hexagon on map
...
for i in range(1):
    ...
    this_region_type = data["Type of place of residence"]==i+1
    ...
    for j in range(Nprov):
        ...
        rwi=wealth_index[in_province]
        eu=energy_use[in_province]
        w=weight[in_province]

        Nh = rwi.size

        o = np.argsort(rwi) # Find indices that order the survey households by ascending rwi
        ...
        # Create overlapping groups of points with ascending average rwi
        Ng = min(int(Nh/4),1000)
        step = int((Nh-Ng)/(Nb-1))
        ...
        wi_group = np.append(np.mean(rwi[group],axis=1),np.mean(rwi[group_high_rwi]))
        ...
        energy_demand[i,include] = np.interp(data_subregions['rwi'][include],wi_group,eu_group)
        ...
        if make_figure:
            if j>0:
                ...
                plt.hist(rwi,bins=bx,weights=w,density=True,alpha=al,edgecolor='b',facecolor='None')
                plt.xlim(xl)
                plt.xticks([])
                plt.ylim(0,1.7)
                ...
                plt.xlim(xl)
                plt.ylim(yl)
                plt.scatter(rwi,eu,s=w,c='c',alpha=al,edgecolors='None')
                plt.text(xl[0],yl[-1],'\n  '+province_list[j],va='top')

                plt.plot(rwi.mean(),eu.mean(),'+',color='g',ms=20)
                ...
                plt.scatter(wi_group,eu_group,s=w_group,alpha=al,marker='d',c='None',edgecolors='b')
                plt.scatter(data_subregions['rwi'][include],energy_demand[i,include],alpha=al,marker='o',s=1,c='orange',edgecolors='None')                
                
            else:
                wi_nat = wi_group.copy()
    ...
    column_out = 'Mean '+region_type[i].lower()+' household energy demand, kWh'
